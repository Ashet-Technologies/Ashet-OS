//!
//! This file contains all definitions and assignments for pins,
//! peripheral usage, ...
//!
//! This way, we can maintain it at a central place.
//!
const std = @import("std");

const machine = @import("ashet-hc.zig");
const rp2350 = @import("rp2350-hal");

pub const clock_config = rp2350.clock_config;

pub const cfg = struct {
    pub const propio_buffer_size = 256;
    pub const propio_buffer_count = 32;

    pub const debug_baud = 2_000_000;
    pub const propeller2_p2boot_baud = 500_000;
    pub const propeller2_propio_baud = 5_000_000;

    pub const default_baud = 115_200;
};

pub const pins = struct {
    // 0
    pub const trace_clk: rp2350.gpio.Pin = rp2350.gpio.num(1); //
    pub const trace_data0: rp2350.gpio.Pin = rp2350.gpio.num(2); //
    pub const trace_data1: rp2350.gpio.Pin = rp2350.gpio.num(3); //
    pub const trace_data2: rp2350.gpio.Pin = rp2350.gpio.num(4); //
    pub const trace_data3: rp2350.gpio.Pin = rp2350.gpio.num(5); //
    pub const i2s_bclk: rp2350.gpio.Pin = rp2350.gpio.num(6); //
    pub const i2s_lrclk: rp2350.gpio.Pin = rp2350.gpio.num(7); //
    pub const xip_cs1: rp2350.gpio.Pin = rp2350.gpio.num(8); //
    pub const i2s_mclk: rp2350.gpio.Pin = rp2350.gpio.num(9); //
    pub const i2s_sdo: rp2350.gpio.Pin = rp2350.gpio.num(10); //
    pub const i2s_sdi: rp2350.gpio.Pin = rp2350.gpio.num(11); //
    pub const hdmi_d0_p: rp2350.gpio.Pin = rp2350.gpio.num(12); //
    pub const hdmi_d0_n: rp2350.gpio.Pin = rp2350.gpio.num(13); //
    pub const hdmi_clk_p: rp2350.gpio.Pin = rp2350.gpio.num(14); //
    pub const hdmi_clk_n: rp2350.gpio.Pin = rp2350.gpio.num(15); //
    pub const hdmi_d1_p: rp2350.gpio.Pin = rp2350.gpio.num(16); //
    pub const hdmi_d1_n: rp2350.gpio.Pin = rp2350.gpio.num(17); //
    pub const hdmi_d2_p: rp2350.gpio.Pin = rp2350.gpio.num(18); //
    pub const hdmi_d2_n: rp2350.gpio.Pin = rp2350.gpio.num(19); //
    pub const clock_src: rp2350.gpio.Pin = rp2350.gpio.num(20); //
    pub const prop_rst: rp2350.gpio.Pin = rp2350.gpio.num(21); //
    pub const i2c_sda: rp2350.gpio.Pin = rp2350.gpio.num(22); // SDA1
    pub const i2c_scl: rp2350.gpio.Pin = rp2350.gpio.num(23); // SCL1
    pub const prop_txd: rp2350.gpio.Pin = rp2350.gpio.num(24); // tx data
    pub const prop_rxd: rp2350.gpio.Pin = rp2350.gpio.num(25); // rx data
    pub const prop_txh: rp2350.gpio.Pin = rp2350.gpio.num(26); // tx handshake
    pub const prop_rxh: rp2350.gpio.Pin = rp2350.gpio.num(27); // rx handshake
    pub const prop_txf: rp2350.gpio.Pin = rp2350.gpio.num(28); // tx frame
    pub const prop_rxf: rp2350.gpio.Pin = rp2350.gpio.num(29); // rx frame
    pub const prop_aux0: rp2350.gpio.Pin = rp2350.gpio.num(30); //
    pub const prop_aux1: rp2350.gpio.Pin = rp2350.gpio.num(31); //
    pub const eth_miso_pin: rp2350.gpio.Pin = rp2350.gpio.num(32); //
    pub const eth_cs_pin: rp2350.gpio.Pin = rp2350.gpio.num(33); //
    pub const eth_sck_pin: rp2350.gpio.Pin = rp2350.gpio.num(34); //
    pub const eth_mosi_pin: rp2350.gpio.Pin = rp2350.gpio.num(35); //
    pub const eth_irq_pin: rp2350.gpio.Pin = rp2350.gpio.num(36); //
    // 37
    pub const uart1_tx: rp2350.gpio.Pin = rp2350.gpio.num(38); //
    pub const uart1_rx: rp2350.gpio.Pin = rp2350.gpio.num(39); //
    pub const userport0: rp2350.gpio.Pin = rp2350.gpio.num(40); //
    pub const userport1: rp2350.gpio.Pin = rp2350.gpio.num(41); //
    pub const userport2: rp2350.gpio.Pin = rp2350.gpio.num(42); //
    pub const userport3: rp2350.gpio.Pin = rp2350.gpio.num(43); //
    pub const uart0_tx: rp2350.gpio.Pin = rp2350.gpio.num(44); //
    pub const uart0_rx: rp2350.gpio.Pin = rp2350.gpio.num(45); //
    pub const debug_core0_tx: rp2350.gpio.Pin = rp2350.gpio.num(46); //
    pub const debug_core1_tx: rp2350.gpio.Pin = rp2350.gpio.num(47); //
};

pub const irq = struct {
    pub const propio_dma: machine.IRQ = .DMA_IRQ_0;
    pub const propio_pio: machine.IRQ = .PIO0_IRQ_0;

    pub const video_dma: machine.IRQ = .DMA_IRQ_1;
};

pub const uart = struct {
    pub const com1 = rp2350.uart.instance.UART0;
    pub const com2 = rp2350.uart.instance.UART1;

    // TODO: Refactor this into using `pio.propio` instead of the dedicated UART:
    pub const propeller2: rp2350.uart.UART = rp2350.uart.instance.UART1;
};

pub const pio = struct {
    pub const propio = rp2350.pio.num(0); // High Speed Southbridge Link
    pub const debuglog = rp2350.pio.num(1); // Dual High Speed UART OUT
    pub const userport = rp2350.pio.num(2); // Free for user / Frontpanel
};

pub const dma = struct {
    pub const hdmi_ping = rp2350.dma.channel(0);
    pub const hdmi_pong = rp2350.dma.channel(1);

    pub const prop_rx: rp2350.dma.Channel = rp2350.dma.channel(2);
    pub const prop_tx: rp2350.dma.Channel = rp2350.dma.channel(3);
};

pub const spi = struct {
    pub const ethernet = rp2350.spi.instance.num(0);
    pub const userport = rp2350.spi.instance.num(1);
};

pub const i2c = struct {
    pub const userport = rp2350.i2c.instance.I2C0;
    pub const system_bus = rp2350.i2c.instance.I2C1;
};

pub const i2c_addresses = struct {
    pub const expansion_eeprom: rp2350.i2c.Address = .new(0x57);

    pub const ds1307_rtc: rp2350.i2c.Address = .new(0x68);
    pub const i2c_main_mux: rp2350.i2c.Address = .new(0x70);
};

pub const adc_channels = struct {
    pub const userport0: rp2350.adc.Input = .ain0;
    pub const userport1: rp2350.adc.Input = .ain1;
    pub const userport2: rp2350.adc.Input = .ain2;
    pub const userport3: rp2350.adc.Input = .ain3;
};
