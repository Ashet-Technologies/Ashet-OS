...
fn spawnApp(app: AppID, executable: loader.LoadedExecutable) !void {
    const process = try ashet.multi_tasking.Process.spawn(
        app.getName(),
        executable.process_memory,
        @as(ashet.scheduler.ThreadFunction, @ptrFromInt(executable.entry_point)),
        null,
        .{ .stack_size = 512 * 1024 },
    );
