...
pub fn main() !void {
    ...
    var output = try std.fs.cwd().atomicFile(argv[2], .{ .write_buffer = &atomic_file_buffer });
    defer output.deinit();

    const document = schema.value;

    try render_docs(&output.file_writer.interface, allocator, document);
...
pub fn render_docs(writer: *std.Io.Writer, allocator: std.mem.Allocator, schema: model.Document) !void {
    try writer.writeAll(
        \\<!doctype html>
        \\<html lang="en">
        \\
    ...
    defer renderer.stack.deinit(allocator);
...
const HtmlRenderer = struct {
    ...
    pub fn render_group(html: *HtmlRenderer, decl: model.Declaration, title: []const u8, tag: model.Declaration.Kind) !void {
        ...
        for (decl.children) |child| {
            ...
            try html.writer.print(
                \\                    <dt><code><span class="tok-kw">{s}</span> <a href="#ashet.{f}"><span class="tok-name">{s}</span></a>(
            , .{
                @tagName(child.data),
                fmt_fqn(child.full_qualified_name),
                child.full_qualified_name[child.full_qualified_name.len - 1],
    ...
    fn fmt_type(html: *HtmlRenderer, type_id: model.TypeIndex) HtmlTypeFmt {
    ...
    const HtmlTypeFmt = struct {
        ...
        pub fn format(self: HtmlTypeFmt, writer: *std.Io.Writer) !void {
            ...
            switch (type_ref) {
                ...
                .@"struct" => |index| try fmt_known_type(writer, self.html.schema.structs[@intFromEnum(index)]),
                .@"union" => |index| try fmt_known_type(writer, self.html.schema.unions[@intFromEnum(index)]),
                .@"enum" => |index| try fmt_known_type(writer, self.html.schema.enums[@intFromEnum(index)]),
                .bitstruct => |index| try fmt_known_type(writer, self.html.schema.bitstructs[@intFromEnum(index)]),
                .resource => |index| try fmt_known_type(writer, self.html.schema.resources[@intFromEnum(index)]),
    ...
    fn fmt_value(html: *HtmlRenderer, value: model.Value) std.fmt.Formatter(model.Value, format_value) {
        _ = html;
        return .{ .data = value };
    }

    fn fmt_known_type(writer: *std.Io.Writer, known_type: anytype) !void {
        try writer.print("<a href=\"#ashet.{f}\"><span class=\"tok-type\">{f}</span></a>", .{
