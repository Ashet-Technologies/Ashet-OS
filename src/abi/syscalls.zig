...
const video = struct {
    ...
    fn set_palette(*VideoOutput, *const [palette_size]Color) SetPaletteError;
...
const network = struct {
    ...
    const udp = struct {
        fn create_socket(result: **UdpSocket) abi.udp.CreateError.Enum;
    };

    const tcp = struct {
        fn create_socket(out: **TcpSocket) abi.tcp.CreateError.Enum;
    };
};
...
const service = struct {
    ...
    fn create(svc: **Service, uuid: *const UUID, funcs: []const AbstractFunction, name: []const u8) ServiceRegisterError;
...
const clipboard = struct {
    ...
    fn get_value(mime: []const u8, value: *?[]const u8) ClipboardGetError;
};
...
const draw = struct {
    ...
    fn get_system_font(font_name: []const u8, font: **Font) GetSystemFontError;

    /// Creates a new custom font from the given data.
    fn create_font(data: []const u8, font: **Font) CreateFontError;
...
const gui = struct {
    ...
    fn register_widget_type(uuid: *const UUID, *const WidgetDescriptor) RegisterWidgetTypeError;

    fn unregister_widget_type(uuid: *const UUID) void;

    // Window API:

    fn create_window(window: **Window, desktop: *Desktop, title: []const u8, min: Size, max: Size, startup: Size, flags: CreateWindowFlags) CreateWindowError;
    ...
    fn create_widget(widget: **Widget, window: *Window, uuid: *const UUID) CreateWidgetError;
...
const notification = struct {
    fn send(message: []const u8, kind: NotificationKind) SendNotificationError;
